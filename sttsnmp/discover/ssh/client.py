...
class SSHClient:
    ...
    def connect(self) -> None:
        ...
        logger.debug(f"  username: {self.config.username!r}")
        logger.debug(f"  password: {'<set>' if self.config.password else '<none>'}")
        logger.debug(f"  key_file: {self.config.key_file or '<none>'}")
        logger.debug(f"  key_content: {'<set>' if self.config.key_content else '<none>'}")
        logger.debug(f"  legacy_mode: {self.config.legacy_mode}")
        ...
        if self.config.key_content or self.config.key_file:
            ...
            logger.debug(f"  loaded key: type={pkey.get_name()}, bits={pkey.get_bits()}, "
                         f"fingerprint={pkey.get_fingerprint().hex()}")
            if self.config.password:
                connect_params['password'] = self.config.password
                logger.debug("  auth strategy: key + password fallback")
            else:
                logger.debug("  auth strategy: key only (no password fallback)")
        else:
            connect_params['password'] = self.config.password
            logger.debug("  auth strategy: password only")

        # Connect with fallback for SHA2 RSA
        try:
            logger.debug(f"  attempt 1: disabled_algorithms={connect_params.get('disabled_algorithms')}")
            self._client.connect(**connect_params)
        except Exception as e:
            logger.debug(f"  attempt 1 failed: {e}")
            logger.debug("  attempt 2: retrying with SHA2 RSA algorithms enabled")
    ...
    def find_prompt(self, attempt_count: int = 3, timeout: float = 3.0) -> str:
        """
