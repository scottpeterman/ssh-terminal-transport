...
TCP reads can be large. DATA payloads are chunked at 16KB before base64 encoding (~21KB on the wire per frame). This keeps individual frames well under the 200KB overflow limit and prevents one bulk transfer from blocking other streams on the terminal channel.
...
## Future: SOCKS5 proxy mode

The current implementation uses static port-mapped routing. A natural extension is a SOCKS5 proxy on a single local port:

```
localhost:1080 → SOCKS5 → dynamic target resolution → SSH terminal → remote TCP
```

This would allow any SOCKS-aware tool (browsers, curl with `--proxy`, proxychains) to reach arbitrary hosts through the tunnel without preconfigured port mappings. The OPEN message already carries the target host:port — the only addition is the SOCKS5 negotiation on the local side.