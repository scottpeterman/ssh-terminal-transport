...
class LegacySSHSupport:
    ...
    @staticmethod
    def configure_modern_algorithms():
        """Set algorithm preferences for modern SSH servers (macOS, Linux)."""
        paramiko.Transport._preferred_kex = (
            "curve25519-sha256@libssh.org",
            "ecdh-sha2-nistp256",
            "ecdh-sha2-nistp384",
...
class SSHClient:
    ...
    def connect(self) -> None:
        ...
        # Connect with fallback for SHA2 RSA, then modern algorithms
        try:
            self._client.connect(**connect_params)
        except Exception as e:
            logger.debug(f"Initial connect failed: {e}, retrying with SHA2 RSA enabled")
            connect_params.pop('disabled_algorithms', None)
            try:
                self._client.connect(**connect_params)
            except Exception as e2:
                logger.debug(
                    f"SHA2 RSA retry failed: {e2}, "
                    "falling back to modern algorithm set"
                )
                LegacySSHSupport.configure_modern_algorithms()
                self._client.connect(**connect_params)
